...
void ARTSPConnection::connect(const char *url, AMessage* reply) {
    AMessage* msg = new AMessage(kWhatConnect, id());
    msg->setString("url", url);
    msg->setMessage("reply", reply);
    msg->post();
}

void ARTSPConnection::disconnect(AMessage* reply) {
    AMessage* msg = new AMessage(kWhatDisconnect, id());
    msg->setMessage("reply", reply);
    msg->post();
}

void ARTSPConnection::sendRequest(
        const char *request, AMessage* reply) {
    AMessage* msg = new AMessage(kWhatSendRequest, id());
    msg->setString("request", request);
    msg->setMessage("reply", reply);
    msg->post();
}

void ARTSPConnection::observeBinaryData(AMessage *reply) {
    AMessage* msg = new AMessage(kWhatObserveBinaryData, id());
    msg->setMessage("reply", reply);
    msg->post();
}

void ARTSPConnection::onMessageReceived(AMessage *msg) {
    switch (msg->what()) {
        case kWhatConnect:
            onConnect(msg);
            break;
...
void ARTSPConnection::onConnect(AMessage* msg) {
    ++mConnectionID;
    ...
    if (err < 0) {
        if (errno == EINPROGRESS) {
            AMessage* msg = new AMessage(kWhatCompleteConnection, id());
            msg->setMessage("reply", reply);
...
void ARTSPConnection::onDisconnect(*AMessage* msg) {
